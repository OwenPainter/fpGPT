...
- `compile.py` does not yet emit the board parameters (`ROM_DEPTH`,
  `W_ADDR_WIDTH`, shifts) or a configured `generation_controller`/engine
  wrapper, so `fpga_top.v`'s defaults must be kept in sync by hand.
- `hdl/gpt_controller.v` and `hdl/transformer_block.v` are now-unused stubs and
  are not in the Quartus project.
- `model/fixed_point.py` is missing the integer-reference API imported by
  `model/validate_fixed.py` and `tests/test_fixed_point.py`, so those fail at
  import.
...
- `python3 -m unittest discover -s tests -v` — collects everything; currently
  fails because `tests/test_fixed_point.py` imports an API that
  `model/fixed_point.py` does not define.
...
- [ ] Restore the `model/fixed_point.py` integer-reference API
...
- [ ] Reconcile `transformer_engine.v` / `generation_controller.v`
      ...
      mixed-width ROM (64-bit biases, 32-bit Q14 gamma / 32-bit beta)
